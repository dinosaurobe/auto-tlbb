...
func runApi(wg *sync.WaitGroup) {
	...
	app.Get("/thienlongbb", func(ctx iris.Context) {
		fmt.Println(ctx)
	...
	app.Get("/thienlongbb/nier.php", func(ctx iris.Context) {
	...
	app.Get("/thienlongbb/scripts.xml", func(ctx iris.Context) {
		ctx.Writef(conf.Mission)
	})

	app.Get("/thienlongbb/map.ini", func(ctx iris.Context) {
		ctx.Writef(conf.Map)
	})
